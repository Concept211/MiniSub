...
function generateAlbumHTML(childid, parentid, coverart, title, artist, rating, starred, created) {
    ...
    if (coverart == undefined) {
        html += '<td class=\"albumart\"><img src=\"images/album.png\" /></td>';
    } else {
        html += '<td class=\"albumart\"><img src=\"' + baseURL + '/getCoverArt.view?' + baseParams + '&size=75&id=' + coverart + '\" /></td>';
    }
...
function generateSongHTML(childid, parentid, artistid, track, title, description, artist, album, coverart, rating, starred, duration) {
    ...
    if (coverart == undefined) {
        ...
        coverartSrc = baseURL + '/getCoverArt.view?' + baseParams + '&size=28&id=' + coverart;
    }
