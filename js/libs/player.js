    ...
    $.ajax({
        ...
        success: function (data) {
            ...
            if (typeof data["subsonic-response"].song != "undefined") {
				...
				if (year !== undefined && year != '') { album += ' (' + year + ')'; }
            }
...
function playSong(el, songid, albumid, title, artist, album, coverart, rating, starred, contenttype, suffix, specs, position, loadonly) {
        ...
        $('#songdetails_artist').attr('title', toHTML.un(artist));
		$('#songdetails_album').html(album);
		$('#songdetails_album').attr('title', toHTML.un(album));
        $('#songdetails_specs').html(specs);
        var coverartSrc, coverartFullSrc;
        if (coverart == undefined) {
            coverartSrc = 'images/album.png';
            coverartFullSrc = '';
        } else {
		...
		$('#preview_song').html(title);
        $('#preview_song').attr('title', title);
        $('#preview_song').attr('parentid', albumid);
        $('#preview_song').attr('childid', songid);
        $('#preview_artist').html(artist);
		$('#preview_album').html(album);
        $('#preview_specs').html(specs);
        if (coverart == undefined) {
            coverartSrc = 'images/album.png';
        } else {
            coverartSrc = baseURL + '/getCoverArt.view?' + baseParams + '&size=500&id=' + coverart;
        }
        $('#preview_coverartimage').attr('href', coverartFullSrc);
        $('#preview_coverartimage img').attr('src', coverartSrc);
		$('#preview_coverartimage img').attr('title', toHTML.un(title) + '\n' + toHTML.un(artist) + '\n' + toHTML.un(album) + '\n' + toHTML.un(specs));
