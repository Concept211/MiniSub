...
function getMusicDirectory(id, action, appendto, artistid) {
	//console.log(baseURL + '/getMusicDirectory.view?' + baseParams + '&id=' + id);
    $.ajax({
            ...
            if (data["subsonic-response"].directory.child !== undefined) {
                ...
                if (appendto == '#AlbumContainer tbody' && isDir == false) {
                    ...
                    if (typeof artistid == 'undefined' || artistid == '') {
                        html = '<span>' + artist + '</span> &raquo;<a href=\"\" albumid=\"' + parentid + '\">' + album + '</a>'
                    } else {
                        html = '<a href=\"\" parentid=\"' + artistid + '\">' + artist + '</a> &raquo;<a href=\"\" albumid=\"' + parentid + '\">' + album + '</a>'
                    }
...
function previewStarredCoverArt() {
    $.ajax({
        url: baseURL + '/getStarred.view?' + baseParams + '&size=28',
        method: 'GET',
        dataType: protocol,
        timeout: 10000,
        success: function (data) {
                ...
                if (data["subsonic-response"].nowPlaying.entry === undefined) {
                    ...
                    $.each(sorted, function (i, msg) {
                        ...
                        if (msg.coverArt === undefined) {
                            ...
                            coverartSrc = baseURL + '/getCoverArt.view?' + baseParams + '&size=48&id=' + msg.coverArt;
                        }
...
function countCurrentPlaylist(container) {
    ...
    if (total > 0) {
        ...
        return total + ' Song(s) / ' + secondsToTime(time) + ' Total';
    } else {
        return '';
    }
}
                ...
                if (data["subsonic-response"].videos != '') {
                    ...
                    $.each(videos, function (i, video) {
                        ...
                        if (video.coverArt === undefined) {
                            ...
                            coverartSrc = baseURL + '/getCoverArt.view?' + baseParams + '&size=28&id=' + video.coverArt;
                        }
